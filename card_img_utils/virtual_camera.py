...
if __name__ == '__main__':
    ...
    while True:
        ...
        edge_src_points = np.array([(0, 0), (src.shape[1], 0), (src.shape[1], src.shape[0]), (0, src.shape[0])],dtype=np.float32)
        dst = cv2.warpPerspective(src, H, (w, h), flags=cv2.INTER_CUBIC)
        edge_dst_points = cv2.perspectiveTransform(edge_src_points[None, :, :], H).squeeze().astype(np.uint)
